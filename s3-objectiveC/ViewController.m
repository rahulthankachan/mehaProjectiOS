...
- (void)uploadToS3{
    ...
    // set the bucket
    _uploadRequest.bucket = @"mbucketdemo542";
    ...
    [[transferManager upload:_uploadRequest] continueWithExecutor:[BFExecutor mainThreadExecutor] withBlock:^id(BFTask *task) {
        ...
        if (task.error) {
            ...
            NSLog(@"https://s3.amazonaws.com/mbucketdemo542/foldername/image.png");
        }
...
- (void) update{
    CGFloat updater= ((float)self.amountUploaded/ (float)self.filesize) * 100;
    if (updater < 100) {
